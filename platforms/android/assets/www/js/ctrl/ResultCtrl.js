...
.factory('SearchResultData', function(search, favorites, catalog, $timeout) {
    ...
    return {
      ...
      setFavorite: function(isFavorite) {
        if (isFavorite && !favorite) {
          favorite = favorites.addSearch(query, projectId);
        } else if (!isFavorite) {
...
.factory('QueryResultData', function(query, favorites, catalog, $q, $timeout) {
  return function createQueryResultData(projectId, queryId) {
    ...
    return {
      ...
      setFavorite: function(isFavorite) {
        if (isFavorite && !favorite) {
          favorite = favorites.addQuery(projectId, queryId);
        } else if (!isFavorite) {
          favorites.remove(favorite);
          favorite = null;
        }
